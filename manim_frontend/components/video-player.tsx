...
export function VideoPlayer({ videoId }: VideoPlayerProps) {
  ...
  useEffect(() => {
    async function fetchPresignedUrl() {
      setIsLoading(true)
      try {
        const response = await fetch(`${API_URL}/api/get_presigned_url/${videoId}`)
  ...
  return (
    <Card className="overflow-hidden border border-border/40 bg-card/30 backdrop-blur shadow-lg rounded-xl">
      ...
      {/* Optional: Video title or additional info could go here */}
      <div className="p-4 space-y-2 hidden">
        <h3 className="text-lg font-medium">Video Title</h3>
        <p className="text-sm text-muted-foreground">Additional information about the video</p>
      </div>
    </Card>
  )
}
